...
def TruncateC2(C2, Chi, D2, cut):
    ...
    # scipy.linalg.eig(C2, right = True)
    s, u = spla.eigs(C2s, k=min(cut+5, C2.shape[0]-2))
...
def Hamiltonians(ds):
    ...
    Sij = 0.5*(Spm + Smp) + Szz
    # (0.5*(Spp + Smm) -Szz).reshape(ds*ds,ds*ds)
    Sijp = (-0.5*(Spp + Smm) - Szz).reshape((ds*ds, ds*ds))
...
def FullUpdate(FullE, V, W, Wp):
    ...
    print(len(np.nonzero(np.round(S, decimals=5))[0])), (len(
        np.nonzero(np.round(R, decimals=5))[0]))
    ...
    print(np.linalg.norm(Rmatrix), np.linalg.norm(
        Smatrix), np.linalg.norm(Amatrix), 'norms')
...
def iPEPSRecycle(E, Corner, TEdge, D, D2, Chi, Chimax, tol, Nsteps):
    ...
    for isteps in range(Nsteps):
        ...
        Corner2 = buildC2(Corner, TEdge, TEdge2, Chi,
                          D2)  # zz((Chi*D2, Chi*D2))
        # print(Corner2)
...
def SaveEnv(Corner, TEdge, Chimax, tol):
    ...
    for i in range(Corner.shape[0]):
        ...
        f.write(str(i)+'\t'+str(Corner[i, i].real/abs(Corner[0, 0])
                                ) + '\t' + str(img/abs(Corner[0, 0]))+'\n')
    f.close()
...
def GetEnv(E, Corner, TEdge, D, D2, Chi, Chimax, tol, Nsteps, Read, Recycle, Input, Saves):
    D2 = D*D
    if Input:
        (Corner, TEdge, Chi, steps) = iPEPSRecycle(
            E, Corner, TEdge, D, D2, Chi, Chimax, tol, Nsteps)
    ...
    if not Input and os.path.isfile('Environment/Corner'+str(Chimax)) and Read:
        ...
        for lines in f1:
            line = lines.split()
            i1, i2, i3, realvalue, imagvalue = int(line[0]), int(
                line[1]), int(line[2]), float(line[3]), float(line[4])
...
def Ttemp(T, LM, RM):
    ...
    temp = tsum('aij, kim, njo -> kanmo', T,
                LM.reshape(D2, Chi, Chi), LM.reshape(D2, Chi, Chi))
...
def FullUEnv(C, T, A, A1):
    ...
    # tsum('aijkl, amnop -> imjnkolp', A, np.conjugate(A1)).reshape(Ddk0*Ddk,D2,D2,D2)
    E = EAcustom(A, A1)
    ...
    # .reshape(ds*D*ds*D, ds*D*ds*D)
    Rho = tsum('iaj, ibj  -> ab',  Ctop, Ctop)
...
def FullUEnv_H(C, T, A):
    ...
    E = tsum('aijkl, amnop -> imjnkolp', A,
             np.conjugate(A)).reshape(Ddk*Ddk, D2, D2, D2)
...
def purif(Rho0):
    ...
    Rtak = np.eye(Ddk)
    RhoTnew = tsum('ikjl ,ia,  kc, jb,  ld ->acbd', Rho0, Rtak,
                   np.conjugate(Rtak), Rtak,  np.conjugate(Rtak))
...
def Optimise(X0, Var):
    ...
    Result = opt.minimize(Fidelity, X0, args=(
        Var,), method='CG', options=options)  # ; Result = Result.x
    return Result.x
...
def XEnv(Corner, T, A, h2, U):
    ...
    R1 = tsum('ab, kbc, lad, ijkl ->dijc', Corner, T,
              T, E2u).reshape(Chi, D, D, D, D, Chi)
...
def Hop(Corner0, TEdge0, Tensors, A0, H):
    ...
    h4 = (h4 + h4.transpose(0, 1, 3, 4, 5, 2) + h4.transpose(0,
                                                             1, 4, 5, 2, 3) + h4.transpose(0, 1, 5, 2, 3, 4))/4.

    Xin = XfromA(A0, Tensors)
    Xin = Xin/np.max(Xin)
    #A = Afromlambda(X,Tensors)
    A = A0
    print('Xinitial ::', Xin)
    ...
    for i in range(1):
        ...
        X = XfromA(A, Tensors)
        X = X/np.max(X)
        Asimple = tsum('yxk, xabcd -> ykabcd', Hsimple,
                       A).reshape(ds, D, D, D, D)
        A1 = tsum('yxk, xabcd -> ykabcd', h1, A).reshape(ds, dk*D, D, D, D)
        A4 = tsum('yxijkl, xabcd -> yiajbkcld', h4,
                  A).reshape(ds, Dd2, Dd2, Dd2, Dd2)
        E1 = EAcustom(A1, A)
    ...
    ###############
        ...
        Rhonew = tsum('abcd, ai, bj, ck, dl -> ijkl', RhoT, U2,
                      np.conjugate(U0), U2, np.conjugate(U0))
        ...
        # sqrtm(RhoAnew @ scipy.linalg.inv(RhoA))
        X = np.diag(np.sqrt(np.diag(RhoAnew) / np.diag(RhoA)))
