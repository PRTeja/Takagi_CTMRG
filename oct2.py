...
def A2B2fromlambda(X, Tensors,Bond,D,ds):
		...
		A2    = Afromlambda(X, Tensors,Bond,D,ds) #tsum('i,ijklmn -> jklmn', Tmv, Tensors)
...
def EnCal(X, H, CTMEnv, Tensors, Bond,D2,ds):
	...
	Eab2t = tsum('abijklmn, abopqrst -> iojpkqlrmsnt', Ab2t,np.conjugate(AB)).reshape(D2,D2,D2,D2,D2,D2)
...
if 1:
	...
	if 1:
		...
		(Corner, TEdge, Chi)=	GetEnv(E, E,E, D, D2,Chi, Chimax, tol, Nsteps, True, True, False,True)
	
		norm, C1, C2 = Calculate2norm(Corner, TEdge, E, Chi, D2)#Calculate2norm(Corner, TEdge, E, Chi, D2)
	...
	""
	dtau =  [0.1,  0.01]#[0.05, 0.025, 0.01, 0.0075, 0.0050, 0.0025, 0.001]#[0.125 - i*0.005 for i in range(25)]
	ncounts= [int(5./i)+1 for i in dtau]#[1,2,10,10]#[3, 7, 10, 25, 50, 50, 50, 50]
	...
	for dt0 in dtau:
		...
		for i in range(Ntau):
			...
			Anew, Fid, Fid2     = SingleOptimisation(Anew,Corner, TEdge, gate, Tensors)
			
			#Anew = Afromlambdanew(Xnew, Tensors)
			Enew = EAcustom(Anew, Anew)
			print(np.allclose(Anew, A))
			(Corner, TEdge, Chi)=	GetEnv(Enew, Corner, TEdge,D, D2,Chi, Chimax, tol, Nsteps, False, True, True, False)
			norm, C1, C2 = Calculate2norm(Corner, TEdge, Enew, Chi, D2)#Calculate2norm(Corner, TEdge, E, Chi, D2)
			#CTMEnv          = FullEnv(C1,C2,D,ds)/norm
			...
			#print('new ::', Xnew)
			En = Calculate2(Corner, TEdge, E2Au,E2Bu, Chi, D2, ds, C1, C2)/norm
		...
		plt.xlabel(r'$\t$')
		plt.yscale('log')
		plt.plot(timing[count], SiSj[count], '-x', label = r'$\tau$ = '+str(np.round(dtau[count],5)))
	...
	Labels = [r'$\lambda^{real}_1$',r'$\lambda^{imag}_1$', r'$\lambda^{real}_2$',r'$\lambda^{imag}_2$']
	...
	for i in iss:
		plt.plot(timing[i], SiSj[i], '-', label = r'$\tau$ = '+str(np.round(dtau[i],5)))
	#plt.plot(dtau, Fid2list, '-o', label = 'full-step')
	plt.legend(loc = 'best')
	plt.savefig('Et.png')
	plt.show()
